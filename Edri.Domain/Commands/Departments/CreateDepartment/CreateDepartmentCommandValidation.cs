using FluentValidation;

namespace Edri.Domain.Commands.Departments.CreateDepartment;

public sealed class CreateDepartmentCommandValidation : AbstractValidator<CreateDepartmentCommand>
{
    public CreateDepartmentCommandValidation()
    {
        AddRuleForId();
        AddRuleForName();
        AddRuleForCode();
    }

    private void AddRuleForId()
    {
        RuleFor(cmd => cmd.AggregateId)
            .NotEmpty()
            .WithMessage("The Department Id is required.");
    }

    private void AddRuleForName()
    {
        RuleFor(cmd => cmd.Name)
            .NotEmpty()
            .WithMessage("The Department Name is required.")
            .MaximumLength(100)
            .WithMessage("The Department Name must not exceed 100 characters.");
    }

    private void AddRuleForCode()
    {
        RuleFor(cmd => cmd.Code)
            .NotEmpty()
            .WithMessage("The Department Code is required.")
            .Length(2)
            .WithMessage("The Department Code must be exactly 2 characters (e.g., '15').");
    }
}